...
[Interactive data expolorer](https://shiny.compbio.dundee.ac.uk/private//marek_ubimito_all)
...
We have `r N(readd(kgg_n_prot_basal))` proteins. Out of these, there are `r N(readd(n_kgg_in_mito))` diGly proteins in MitoCarta database. None of them is E2 or E3.
...
For comparison, we use our data with $|\log_2 FC| > 1$ and $FDR < 0.05$. Only proteins present in MitoCarta are shown. Here is the comparison result:

```{r plot_venn, fig.width=5, fig.height=3}
loadd(kgg_ineu_over, plt_venn)
plot(plt_venn, quantities=TRUE) 
```

### Only in KGG significant

```{r kgg_sig}
kgg_ineu_over$vp$kgg_only %>% myKable(col.names="Significant kGG only")
```

### Only in iNeurons

```{r ineu_dig}
kgg_ineu_over$vp$ineu_only %>% myKable(col.names="Significant iNeurons only")
``` 

### Overlap

```{r ineu_kgg}
kgg_ineu_over$vp$overlap %>% myKable(col.names="Overlap")
```
